...
class MSAligner:  # pylint: disable=too-many-instance-attributes
    ...
    def gen_coarse(self, ds1, ds2):
        ...
        if len(res) > 0:
            ...
            self.coarse_matches[ds1][ds2] = None
    ...
    def gen_scaled_values(self, ds1, ds2):
        ...
        if scalers is None:
            self.scaled_values[ds1][ds2] = None
            return
        for descr, mode in self.descriptors.items():
    ...
    def gen_stds(self, ds1, ds2):
        ...
        if coarse_matches is None:
            self.stds[ds1][ds2] = None
            return
        scalers = self.scalers[ds1][ds2]
        self.stds[ds1][ds2] = {}
        for descr, mode in self.descriptors.items():
    ...
    def gen_matches(self, ds1, ds2):
        ...
        cutoffs = {}
        weights = {}
        if self.stds[ds1][ds2] is None:
            self.matches[ds1][ds2] = None
            return
        for descr in self.descriptors:
            try:
    ...
    def gen_graph(self, ds1, ds2):
        ...
        if self.graph_backend == "kbbqgraph":
            ...
            if self.graph is None:
                ...
                # pylint: disable-next=possibly-used-before-assignment,undefined-variable
...
def anchors_mz(ds, match_params, mz_key, remove_all=True, priority="Intensity"):
    ...
    for descriptor in thresholds:
        ...
        if match_params[descriptor]["mode"] == "log10":
            ...
            raise NotImplementedError(
                f"Unknown mode: {match_params[descriptor]['mode']}."
            )
    ...
    for i, row in enumerate(ds):
        ...
        for j, param in enumerate(match_params.values()):
            ...
            if param["mode"] == "linear":
                ...
                raise NotImplementedError(f"Unknown mode: {param['mode']}.")
...
def gen_intensity(dataset, intensity_col="Intensity", ignore=None):
    ...
    :param intensity_col: str, column header for the intensity column
    :param ignore: list, columns to ignore when calculating intensity
    """
    calc_df = dataset.drop(columns=ignore) if ignore else dataset.copy()
